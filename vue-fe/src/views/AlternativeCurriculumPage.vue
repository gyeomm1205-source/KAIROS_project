  ...
  <div class="setup-root">
    ...
    <div class="setup-card custom-scroll">
      ...
      <div v-if="isLoading" class="loading-container">
        <h3 class="loading-title">GENERATING ALTERNATIVE...</h3>
        <p class="loading-subtitle">다른 방향의 학습 계획을 설계 중입니다.</p>
...
const currProgress = computed(() => ((currLoadStep.value + 1) / 3) * 100)

const CURR_STEPS = [
  { messages: ['분석 결과를 읽고 있어요...', '대안 학습 주제를 탐색 중이에요...'] },
  { messages: ['기존 일정을 확인하고 있어요...', '다른 방향의 배치를 시도 중이에요...'] },
  { messages: ['대체 커리큘럼을 구성 중이에요...', '거의 완성됐어요!'] },
]
...
.loading-title { font-size: 18px; font-weight: 900; letter-spacing: 0.12em; color: var(--text-primary); margin: 0; }
.loading-subtitle { font-size: 13px; font-weight: 600; color: var(--text-muted); margin: 0; }
...
.mock-url { font-size: 11px; font-weight: 700; color: var(--text-muted); display: flex; align-items: center; gap: 6px; }
.mock-calendar-body { padding: 16px; position: relative; overflow: hidden; }
.mock-week-header { display: grid; grid-template-columns: repeat(7, 1fr); gap: 4px; margin-bottom: 8px; }
.mock-week-header span { text-align: center; font-size: 10px; font-weight: 800; color: var(--text-muted); }
.mock-week-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 4px; }
.mock-day { aspect-ratio: 1; border: 1px solid var(--border); border-radius: 4px; display: flex; align-items: center; justify-content: center; padding: 4px; }
.mock-day.has-node { border-color: var(--text-primary); }
.mock-node { width: 100%; display: flex; flex-direction: column; gap: 3px; }
.mock-node-bar { height: 4px; border-radius: 2px; background: var(--text-primary); animation: nodeBarGrow 0.6s ease-out both; }
.mock-node-text { height: 3px; width: 70%; border-radius: 2px; background: var(--text-muted); opacity: 0.2; animation: nodeBarGrow 0.6s ease-out 0.2s both; }
...
@keyframes nodeBarGrow { from { width: 0; opacity: 0; } to { width: 100%; opacity: 1; } }
.mock-scan-line {
  position: absolute; top: 0; left: -100%; width: 100%; height: 100%;
  background: linear-gradient(90deg, transparent, var(--text-primary), transparent);
  opacity: 0.04; animation: scanMove 3s ease-in-out infinite;
}
@keyframes scanMove { 0% { left: -100%; } 50% { left: 100%; } 100% { left: 100%; } }

.curriculum-typing-text { font-size: 14px; font-weight: 700; color: var(--text-primary); min-height: 24px; }
.cursor { color: var(--text-primary); animation: blink 0.8s step-end infinite; font-weight: 400; margin-left: 1px; }
...
.curr-progress-track { width: 100%; height: 3px; background: var(--border); overflow: hidden; border-radius: 2px; }
.curr-progress-fill { height: 100%; background: var(--text-primary); transition: width 0.4s ease-out; border-radius: 2px; }
</style>
