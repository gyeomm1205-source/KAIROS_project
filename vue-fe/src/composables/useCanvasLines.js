...
export const useCalendarStore = defineStore('calendar', () => {
  // ★ 8비트 아케이드 컨셉에 맞춘 쨍하고 대비가 확실한 원색 팔레트
  const tracks = ref([
    { id: 'project',   name: '졸업작품 (Spring Boot)', color: '#1E88E5', index: 0, isEnded: false }, // 파랑 (Retro Blue)
    { id: 'cert',      name: '정보처리기사 준비',      color: '#E53935', index: 1, isEnded: false }, // 빨강 (Retro Red)
    { id: 'algo',      name: '코딩테스트 스터디',      color: '#43A047', index: 2, isEnded: false }, // 초록 (Retro Green)
    { id: 'cs',        name: 'CS 전공지식',            color: '#FBC02D', index: 3, isEnded: false }, // 노랑 (Arcade Yellow)
  ])
  ...
  const allTracks = computed(() => {
    ...
    if (!list.some(t => t.id === 'prompt')) list.push({ id: 'prompt', name: '프롬프트', color: '#8E24AA', index: 98, isHighlight: true, isEnded: false }) // 보라
    if (!list.some(t => t.id === 'blog')) list.push({ id: 'blog', name: '블로그 어시스턴트', color: '#00ACC1', index: 99, isHighlight: true, isEnded: false }) // 청록
    return list.sort((a, b) => a.index - b.index)
  })

  const HIGHLIGHT_TRACKS = [
    { id: 'prompt', name: '프롬프트', color: '#8E24AA' },
    { id: 'blog', name: '블로그 어시스턴트', color: '#00ACC1' }
  ]
